...
export const KycVerification: FC<KycVerificationProps> = ({
  ...
  const handleVerification: FormProps<VerificationRequest>['onFinish'] = async () => {
    try {
      ...
      if (!userInfo?.accountInfo?.accountObjectId) {
        message.error('User information is incomplete. Please try again later.');
        return;
      }
      ...
      const payload: VerificationRequest = {
        accountObjectId: userInfo.accountInfo.accountObjectId,
      ...
      try {
        ...
        message.success('Your verification request has been submitted successfully.');
        handleOk();
        handleCancel();
      } catch (err: any) {
        const errorMsg =
    ...
    if (data.status === 'start') {
      ...
      if (data.status === 'done') {
        if (data.label === 'ID FRONT') {
          setfrontIdFileName(data.data || '');
        } else if (data.label === 'ID BACK') {
          setbackIdFileName(data.data || '');
        } else if (data.label === 'SELFIE') {
          setselfieFileName(data.data || '');
        }
      } else {
  ...
  useEffect(() => {
    if (isModalOpen && userInfo?.accountInfo) {
      const a = userInfo.accountInfo;
      form.setFieldsValue({
        ...
        birthDate: a.birthDate ? dayjs(a.birthDate) : null,
        natureOfWork: a.natureOfWork ?? undefined,
        sourceOfIncome: a.sourceOfIncome ?? undefined,
        salaryRange: a.salaryRange ?? undefined,
      });

      setfrontIdFileName(a.frontIdPath);
      setbackIdFileName(a.backIdPath);
      setselfieFileName(a.selfiePath)
    }
    ...
    >
      <div className="login-header">
        ...
        <Text style={{ color: '#ccc' }}>
          Complete your verification to become a fully verified player
        </Text>
      </div>
      ...
      >
        <Spin spinning={isloading} tip="Uploading...">
          ...
          <div style={{ display: 'flex', gap: '15px', justifyContent: 'space-between' }}>
            <BasicFileUpload label="ID FRONT" callBack={handleUploadCallback} initialBase64={frontImgage} />
            <BasicFileUpload label="ID BACK" callBack={handleUploadCallback} initialBase64={backImage} />
            <BasicFileUpload label="SELFIE" callBack={handleUploadCallback} initialBase64={selfieImage} />
          </div>
